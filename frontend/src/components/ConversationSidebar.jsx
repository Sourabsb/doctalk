...
const ConversationSidebar = ({
  ...
  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-full md:w-80'} bg-gray-900 flex flex-col transition-all duration-300`}>
      ...
      <div className="p-3 space-y-2">
        ...
        {!isCollapsed && (
          <>
            <button
              onClick={onNewChat}
              className="w-full flex items-center space-x-2 px-3 py-2 text-gray-300 hover:text-white hover:bg-gray-800 rounded-lg transition text-sm"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 pl-9 bg-gray-800 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            ...
            {filteredConversations.map((conversation) => (
              <div
                ...
                className={`group relative px-3 py-2.5 rounded-lg transition cursor-pointer ${
                  activeId === conversation.id ? 'bg-gray-800' : 'hover:bg-gray-800/50'
                }`}
                onClick={() => onSelect(conversation.id)}
              >
                <div className="flex items-center justify-between gap-2">
                  <h3 className="text-sm text-gray-200 truncate flex-1">
                    {conversation.title}
                  </h3>
                  <button
                    onClick={(e) => {
        ...
        {!isCollapsed ? (
          <div className="space-y-2">
            <button
              ...
              className="w-full flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-800 transition text-white"
            >
            ...
            {showSettings && (
              <div className="space-y-1 pl-2">
                <button
                  ...
                  className="w-full flex items-center space-x-2 p-2 rounded-lg hover:bg-gray-800 transition text-gray-300 hover:text-white text-sm"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col items-center space-y-3">
            <button
              onClick={() => setShowSettings(!showSettings)}
              className="w-10 h-10 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full flex items-center justify-center hover:opacity-80 transition"
            ...
            {showSettings && (
              <div className="flex flex-col items-center space-y-2">
                <button
                  onClick={onOpenProfileSettings}
                  className="w-10 h-10 rounded-lg hover:bg-gray-800 transition text-gray-300 hover:text-white flex items-center justify-center"
